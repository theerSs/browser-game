export function useCombatFormatting(combatState: Ref<CombatState | null>) {
  ...
  const defenceValue = computed(() => player.value ? `${Math.max(5, player.value.stats.defence)}%` : "");
  const dodgeChance = computed(() => player.value && enemy.value ? `${Math.max(5, player.value.stats.dex - enemy.value.stats.dex)}%` : "");
